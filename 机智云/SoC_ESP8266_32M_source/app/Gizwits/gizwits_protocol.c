...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagnf)
    {
        info->event[info->num] = EVENT_nf;
        info->num++;
        dataPoints->valuenf = gizStandardDecompressionValue(nf_BYTEOFFSET,nf_BITOFFSET,nf_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuenf != cur->valuenf)
    {
        GIZWITS_LOG("valuenf Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(nf_BYTEOFFSET,nf_BITOFFSET,nf_LEN,(uint8_t *)devStatusPtr,dataPoints->valuenf);
